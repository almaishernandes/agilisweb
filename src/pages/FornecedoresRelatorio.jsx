...
const FornecedoresRelatorio = () => {
    const anoAtual = new Date().getFullYear();
    const [ano, setAno] = useState(anoAtual);
    const [periodo, setPeriodo] = useState('mensal');
    const [aba, setAba] = useState('comp');   // 'comp' | 'red'
    ...
    useEffect(() => {
        supabase.from('beneficiaries')
            .select('id, full_code, code, level, name, category')
            .order('full_code')
            .then(({ data }) => {
                ...
                setBeneficiaries(sorted.filter(r => r.full_code?.trim() || r.name?.trim()));
            });
    }, []);
    ...
    const fetchTransactions = async () => {
        ...
        const { data } = await supabase
            .from('transactions')
            .select('due_date, amount, dc_type, beneficiary_id')
        ...
        const { data: anteriores } = await supabase
            .from('transactions')
            .select('amount, dc_type, beneficiary_id')
    ...
    const getCode = (b) => b.full_code || '';
    const getLevel = (b) => Math.max(0, getCode(b).split('.').length - 1);

    const fullMatrix = useMemo(() => {
        const map = {};
        beneficiaries.forEach(b => { map[b.id] = new Array(colunas.length).fill(0); });
        transactions.forEach(t => {
            const id = t.beneficiary_id;
            if (!map[id]) return;
        ...
        return map;
    }, [transactions, beneficiaries, colunas, periodo]);

    const saldoAnteriorRaw = useMemo(() => {
        const map = {};
        beneficiaries.forEach(b => { map[b.id] = [0]; });
        ...
        return map;
    }, [saldoAnteriorTx, beneficiaries]);

    const isReduced = aba === 'red';

    const { displayAccounts, displayMatrix } = useMemo(
        () => buildDisplay(beneficiaries, fullMatrix, colunas.length, isReduced, getCode),
        [isReduced, beneficiaries, fullMatrix, colunas]
    );

    const { displayMatrix: saldoAnteriorMatrix } = useMemo(
        () => buildDisplay(beneficiaries, saldoAnteriorRaw, 1, isReduced, getCode),
        [isReduced, beneficiaries, saldoAnteriorRaw]
    );

    const visibleAccounts = useMemo(() => {
        if (!isReduced) return displayAccounts;
        return displayAccounts.filter(b => (displayMatrix[b.id] || []).some(v => v !== 0));
    }, [displayAccounts, displayMatrix, isReduced]);

    const totais = useMemo(() => {
        const t = new Array(colunas.length).fill(0);
        visibleAccounts.forEach(b => { (displayMatrix[b.id] || []).forEach((v, i) => { t[i] += v; }); });
        return t;
    }, [displayMatrix, visibleAccounts, colunas]);

    const totalSaldoAnterior = useMemo(() => {
        return visibleAccounts.reduce((s, b) => s + ((saldoAnteriorMatrix[b.id] || [0])[0]), 0);
    ...
    const Badge = ({ val }) => (
        <span style={{ fontSize: '9px', fontWeight: 'bold', color: '#ffffff', background: val >= 0 ? '#1b5e20' : '#c62828', borderRadius: '50%', width: '16px', height: '16px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', marginLeft: '4px', verticalAlign: 'middle' }}>
            {val >= 0 ? 'C' : 'D'}
        </span>
    );
    ...
    return (
        <MainLayout>
            <ReportPDF title={`Fornecedores ${ano} — ${aba === 'comp' ? 'Completo' : 'Reduzido'}`}>
                ...
                <div style={{ flex: 1, overflowY: 'auto', overflowX: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
                        <thead style={{ position: 'sticky', top: 0, zIndex: 5 }}>
                            <tr>
                                <th style={{ ...thS, textAlign: 'left', minWidth: '260px', position: 'sticky', left: 0, zIndex: 6 }}>Fornecedor</th>
                        ...
                        <tbody>
                            {visibleAccounts.map((b, i) => {
                                const row = displayMatrix[b.id] || new Array(colunas.length).fill(0);
                                const total = row.reduce((s, v) => s + v, 0);
                                const saldoAnterior = (saldoAnteriorMatrix[b.id] || [0])[0];
                                ...
                                return (
                                    <tr key={b.id} style={{ background: bg }}>
                                        <td style={{ padding: '4px 8px', paddingLeft: `${8 + level * 12}px`, position: 'sticky', left: 0, background: bg, borderRight: '1px solid #eee', color: level === 0 ? '#004d40' : '#444', fontWeight: level === 0 ? '700' : '400', fontSize: '10px', whiteSpace: 'nowrap' }}>
                                            <span style={{ color: '#aaa', marginRight: '5px' }}>{getCode(b)}</span>{b.name}
                                        </td>
                                        ...
                                        <td style={{ padding: '4px 8px', textAlign: 'right', fontWeight: '700', color: total > 0 ? '#1565c0' : total < 0 ? '#c62828' : '#ccc', borderLeft: '2px solid #eee' }}>
                                            {total === 0 ? '—' : <>{fmt(Math.abs(total))} <Badge val={total} /></>}
                                        </td>
                                    </tr>
                                );
                            })}
                        ...
                        <tfoot>
                            <tr style={{ background: '#89962F', fontWeight: '700', position: 'sticky', bottom: 0 }}>
                                ...
                                <td style={{ padding: '6px 8px', textAlign: 'right', color: '#ffffff', borderRight: '2px solid #ccc' }}>
                                    {totalSaldoAnterior === 0 ? '—' : <>{fmt(Math.abs(totalSaldoAnterior))} <Badge val={totalSaldoAnterior} /></>}
                                </td>
                                {totais.map((v, ci) => (
                                    <td key={ci} style={{ padding: '6px 8px', textAlign: 'right', color: '#ffffff' }}>
                                        {v === 0 ? '—' : <>{fmt(Math.abs(v))} <Badge val={v} /></>}
                                    </td>
                                ))}
                                {(() => {
                                    const tot = totais.reduce((s, v) => s + v, 0);
                                    return (
                                        <td style={{ padding: '6px 8px', textAlign: 'right', color: '#ffffff', borderLeft: '2px solid #ccc' }}>
                                            {tot === 0 ? '—' : <>{fmt(Math.abs(tot))} <Badge val={tot} /></>}
                                        </td>
                                    );
                                })()}
                            </tr>
...
export default FornecedoresRelatorio;
